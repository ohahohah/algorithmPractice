/**
<pre>
HackerRank.Regex.Applications.Split the Phone Numbers
</pre>
...
public class SplitThePhoneNumbersSolution {
    ...
    public static void main(String[] args) {
        ...
        //sol01. Scanner.nextLine() -> regExp matche
        //1. STDIN
        Scanner in = new Scanner(System.in);
        int phoneNumCount = in.nextInt();
        in.nextLine();
        String[] strArrPhoneList = new String[phoneNumCount];
        for(int i = 0; i < phoneNumCount; i++){
            strArrPhoneList[i] = in.nextLine();
        }
        in.close();
        
        //2.RegExp matche, STDOUT
        String regExp = "^(\\d{1,3})[-\\s](\\d{1,3})[-\\s](\\d{4,10})$";
        
        Pattern p = Pattern.compile(regExp);

        for(String strPhone : strArrPhoneList){
            Matcher m = p.matcher(strPhone);    
            //if(m.find()){
            if(m.matches()){
                System.out.print("CountryCode=" + m.group(1));        
                System.out.print(",LocalAreaCode=" + m.group(2));        
